...
def _send_gotify(title, message, priority=None):
    ...
    try:
        r = requests.post(f"{GOTIFY_URL}/message", json=payload,
...
def _send_ntfy(title, message, priority=None, tags=None, actions=None, click_url=None):
    ...
    if actions:
        parts = []
        for a in actions:
            if a.get("type")=="view": parts.append(f"view, {a['label']}, {a['url']}")
            elif a.get("type")=="http":
                p = f"http, {a['label']}, {a['url']}"
                if a.get("method"): p+=f", method={a['method']}"
                if a.get("body"): p+=f", body={a['body']}"
                parts.append(p)
        if parts: headers["Actions"] = "; ".join(parts)
    try:
...
def notify_run_complete(run_id, project_name, score, success, language,
    ...
    if troubleshoot_attempts > 0: lines.append(f"**Troubleshoot:** {troubleshoot_attempts}x")
    if deploy_path: lines.append(f"**Deployed:** `{deploy_path}`")
    if not success: lines.append("**Status:** execution failed")
    message = "\n".join(lines)
